...
MODEL_PATH = 'iic/speech_eres2netv2w24s4ep4_sv_zh-cn_16k-common'
...
def process_csv_file(csv_path):
    ...
    for i in tqdm(range(len(df) - 1), desc=f"Processing {csv_path}"):
        spk_1 = df['pred_speaker'].iloc[i]
        spk_2 = df['pred_speaker'].iloc[i+1]
        file1 = df['files'].iloc[i]
        file2 = df['files'].iloc[i+1]

        try:
            result = sv_pipeline([file1, file2])
            score = result['score']
            scores.append(score)
...
if __name__ == '__main__':
    csv_files = [f'pred_res/exp_1/show/{i:02d}.csv' for i in range(1, 21)]

    num_processes = 8 

    with Pool(processes=num_processes, initializer=init_process) as pool:
    ...
    all_scores = (all_scores + 1) / 2

    try:
        auc = roc_auc_score(all_labels, all_scores)
        fnr, fpr = compute_pmiss_pfa_rbst(all_scores, all_labels)
        eer, thres = compute_eer(fnr, fpr, all_scores)
        print(f"Overall AUC: {auc:.4f}")
        print(f"Overall eer: {eer:.4f}")
