...
class VisionProcesser():
    ...
    def run(self):
        ...
        for [audio_sample_st, audio_sample_ed] in self.audio_vad:
            ...
            for _ in range(num_frames):
                ret, frame = self.cap.read() # ret 是一个布尔值，表示是否成功读取帧；frame 是读取到的帧图像。
                if not ret:
                    break
                if index % self.face_det_stride==0: # index 是否是 self.face_det_stride 的倍数。如果是，将当前帧添加到 face_det_frames 列表中，用于后续的人脸检测。
                    face_det_frames.append(frame)
                frames.append(frame)
                if (index + 1) % self.shot_stride==0: # 处理一组帧
    ...
    def track_shot(self, scene_faces):
        # 输入：scene_faces 是一个列表的列表，其中外层列表的每个元素对应一帧，内层列表包含该帧中所有检测到的人脸的边界框（bounding boxes）
       # 输出： track_shot 方法的输出是 tracks，这是一个列表，其中每个元素代表一个连续的人脸轨迹。每个轨迹是一个字典，包含以下键值对：
            #'frame'：一个整数数组，表示轨迹中包含的帧的索引。
            #'bbox'：一个二维数组，其中每一行代表轨迹中每个人脸的边界框坐标。
        # Face tracking
        tracks = []
    ...
    def crop_video(self, track, frames, audio):
        ...
        # x：代表人脸边界框中心的 x 坐标。
        # y：代表人脸边界框中心的 y 坐标。
        # s：代表人脸边界框的大小，通常是边界框宽度和高度的平均值，用于表示人脸的大小。
        crop_frames = []
        dets = {'x':[], 'y':[], 's':[]}
